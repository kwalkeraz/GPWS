<%
    ...
    while(CPRoutingTemplate_RS.next()) {
    	templatename = tool.nullStringConverter(CPRoutingTemplate_RS.getString("TEMPLATE_NAME"));
    	reqtype = tool.nullStringConverter(CPRoutingTemplate_RS.getString("REQUEST_TYPE"));
    	cptemplateid = CPRoutingTemplate_RS.getInt("CP_TEMPLATE_ID");
    }	
%>
	<%@ include file="metainfo.jsp" %>
	<meta name="Keywords" content="Global Print routing template edit"/>
	<meta name="Description" content="Global print website routing template edit page" />
	<title><%= messages.getString("global_print_title") %> | <%= messages.getString("cp_edit_routing_template") %></title>
	...
	<script type="text/javascript">
	 ...
	 function editTemplate(event) {
        ...
        var logaction = "Routing template " + wName + " has been updated";
	 ...
	 dojo.ready(function() {
     	createHiddenInput('topageid','<%= BehaviorConstants.TOPAGE %>', '7484');
        createHiddenInput('logactionid','logaction','');
        createHiddenInput('logactionid','cptemplateid','<%= cptemplateid %>');
        createpTag();
        createTextInput('templatename','templatename','templatename','32',true,'<%= messages.getString("required_info") %>','required','<%= messages.getString("field_problems") %>','^[A-Za-z0-9 _.;,()\/-]*$','<%= templatename %>');
        createSelect('requesttype', 'requesttype', '<%= messages.getString("please_select_value") %>... ', 'None', 'requesttype');
        addRequestType();
        autoSelectValue('requesttype','<%= reqtype %>');
	...
	<div id="ibm-leadspace-head" class="ibm-alternate">
		<div id="ibm-leadspace-body">
			...
			<h1><%= messages.getString("cp_edit_routing_template") %></h1>
		</div>
	</div>
