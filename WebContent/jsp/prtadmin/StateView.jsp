	...
	<meta name="Keywords" content="Global Print add state"/>
	<meta name="Description" content="Global print website add state information" />
	<title><%= messages.getString("title") %> | <%= messages.getString("state_add") %></title>
	...
	<script type="text/javascript">
	 ...
	 function addState(event) {
	 	...
	 	dojo.stopEvent(event);
	 		if (formValid) {
	 			if (!found) {
		 			logaction.value = "State " + wName + " has been added";
					formName.submit();
				} else {
	 ...
	 dojo.ready(function() {
     	createHiddenInput('topageid','<%= BehaviorConstants.TOPAGE %>', '408');
        createHiddenInput('logactionid','logaction','');
        createHiddenInput('logactionid','geo','<%=geo%>');
        createHiddenInput('logactionid','country','<%=country%>');
        createpTag();
        createTextInput('state','state','state','64',true,'<%= messages.getString("required_info") %>>','required','<%= messages.getString("field_problems") %>',g_loc_state_regexp,'');
	...
	<div id="ibm-leadspace-head" class="ibm-alternate">
		<div id="ibm-leadspace-body">
			...
			<h1><%= messages.getString("state_add") %></h1>
		</div>
	</div>
