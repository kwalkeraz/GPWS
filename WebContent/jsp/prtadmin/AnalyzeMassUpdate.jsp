	...
	<title><%= messages.getString("global_print_title") %> | <%= messages.getString("mass_update_analysis") %></title>
	...
	function showHelp() {
		miniwinpopup('/tools/print/MassUpdateHelp.html', 'helpwindow', 650, 400);
		//return false;
	}
	
	function miniwinpopup(myurl, name, x, y){
	               if (navigator.appName == "Netscape"){
	                      if ((navigator.appVersion.substring(0, 3) == "3.0")
	                    || (navigator.appVersion.substring(0, 3) == "4.0")) {
	                            window1 = window.open(myurl, name, 
	                            "width="+x+",height="+y+",resizable=1,status=1,menubar=0,scrollbars=1");
	                            window1.focus();
	                      }
	                      else {  // Netscape 2
	...
	function massUpdate(event) {
	 	var formName = document.getElementById('massUpdateForm');
	 	formName.submit();
	 } //massUpdate
	...
	<div id="ibm-leadspace-head" class="ibm-alternate">
		<div id="ibm-leadspace-body">
			...
			<h1><%= messages.getString("mass_update_analysis") %></h1>
			</div>
		</div>
...
int maxNumColumns = 200;
int x = 0; // x is the number of data entries such as number of devices trying to import.
int a = 0 ,b = 0,c = 0,d = 0,y = 0,z = 0,j = 0,w = 1,linenum = 1,rowcount = 0,rowcounterror = 0,rowcountwarning = 0,lineignore = 0,numCols = 0,iGray = 0;
...
// Was going to start testing a HashMap to track required fields instead of an array. Just haven't really gotten to it yet.
// Map<String, Boolean> prtReqFields = new HashMap<String, Boolean>();
// prtReqFields.put("GEO", false);
// prtReqFields.put("COUNTRY", false);
// prtReqFields.put("STATE", false);
// prtReqFields.put("CITY", false);
// prtReqFields.put("BUILDING_NAME", false);
// prtReqFields.put("FLOOR_NAME", false);
// prtReqFields.put("ROOM", false);
// prtReqFields.put("DEVICE_NAME", false);
// prtReqFields.put("MODEL", false);
// prtReqFields.put("WEB_VISIBLE", false);
// prtReqFields.put("INSTALLABLE", false);
// prtReqFields.put("STATUS", false);
// prtReqFields.put("FTP_SITE", false);
// prtReqFields.put("PRINTER_DEF_TYPE", false);
// prtReqFields.put("DRIVER_SET_NAME", false);
// prtReqFields.put("LPNAME", false);
// prtReqFields.put("DIPP", false);
// prtReqFields.put("DEVICE_FUNCTION", false);
...
try {
	...
	TheBean.setSavePath(System.getProperty("java.io.tmpdir"));
	...
	int maxNumEntries = TheBean.getNumMassUpdates(con) + 1;
	BufferedReader in = new BufferedReader(new FileReader( filename) ) ;
	
	String []errorrows = new String[maxNumEntries];
	String []errormsgs = new String[maxNumEntries];
	String []data = new String[maxNumEntries]; // each data[] entry contains 1 row of values in the csv file
	String []p;  // p contains each column data. So for 1 device, each entry of p contains the column info for that devie.
	String []columns = new String[maxNumColumns];
	String []warnings = new String[maxNumEntries];
	...
	while( ( s = in.readLine()) != null ) {
		if (x >= maxNumEntries) {
			overMax = true;
			break;
		} else if (s != null && !s.substring(0,1).equals("#") && !s.substring(0,2).equals("\"#")) {
			if (colList == false) {
				s = lineCount + "," + s;
			}
			colList = false;
	...
	if(Table.equals("DEVICE") || Table.equals("ENBLHEADER") || Table.equals("ENBLDETAIL") || Table.equals("SERVER")) {
        ...
        columns[w] = (data[0].substring(j,data[0].length())).toUpperCase().trim();
	}
	...
	String[][] dataInfo = new String[x+1][numCols+1];
	String[][] dataInfoOld = new String[x+1][numCols+1];
	
	for(y = 0;y < x;y++) {
		// x = number of devices to be inserted/updated
		deviceFunc = 0;
		locCheck = false;
		ftpCheck = false;
		e2eCheck = true;
		...
		if(Table.equals("DEVICE")) {
			...
			dataInfo[y][0] = p[0];
			dataInfoOld[y][0] = "Old Value";
			for (int count = 1; count < numCols; count++) {
				dataInfo[y][count] = p[count]; %>
<%				if (columns[count].equals("PRINTER_DEF_TYPE") ) {
					...
					try {
						...
						if (bChangeValue == false) {
	  					...
	  					System.out.println("GPWS error in AnalyzeMassUpdate.jsp.8 ERROR: " + e);
	  				} finally {
	  					try {
	  						if (rsType != null)
	  							rsType.close();
		  					...
		  					System.out.println("GPWS Error in AnalyzeMassUpdate.jsp.9 ERROR: " + e);
	  					}
	  				}
					...
					try {
						...
						while (rsPrinterFTP.next()) {
	  					...
	  					System.out.println("GPWS error in AnalyzeMassUpdate.jsp ERROR: " + e);
	  				} finally {
	  					try {
	  						rsPrinterFTP.close();
	  						stmtPrinterFTP.close();
	  					} catch (Exception e){
		  					System.out.println("GPWS Error in AnalyzeMassUpdate.jsp.1 ERROR: " + e);
	  					}
	  				}
	  				if (iFTPID != 0) {
						...
						ResultSet rsPrinterLoc = null;
	  					try {
							...
							while (rsPrinterLoc.next()) {
	  						...
	  						System.out.println("GPWS error in AnalyzeMassUpdate.jsp.2 ERROR: " + e);
	  					} finally {
	  						try {
		  						...
		  						System.out.println("GPWS Error in AnalyzeMassUpdate.jsp.3 ERROR: " + e);
	  						}
	  					}
...
<%			if (Action.equals("INSERT_UPDATE") || Action.equals("INSERT")) {
				...
				try {
					...
					while (rsPrinter.next()) {
  					...
  					System.out.println("GPWS error in AnalyzeMassUpdate.jsp.10 ERROR: " + e);
  				} finally {
  					try {
  						if (rsPrinter != null)
  							rsPrinter.close();
	  					...
	  					System.out.println("GPWS Error in AnalyzeMassUpdate.jsp.11 ERROR: " + e);
  					}
  				}
  
  				if (Action.equals("INSERT_UPDATE")) {
	  				try {
	  					// Code to loop through and get all device old values
	  					//System.out.println("length of dataInfoOld = " + dataInfoOld.length);
	  					//System.out.println("length of dataInfoOld[0] = " + dataInfoOld[0].length);
	  					//System.out.println("Checking if printerCheck is true");
	  					if (printerCheck == true) {
	  						//System.out.println("Printer Check is true");
	  						if (sPrinterName != null && !sPrinterName.equals("")) {
	  							//System.out.println("y = " + y + " and calling FileUploadBean.getDeviceInfo");
		  						dataInfoOld[y] = TheBean.getDeviceInfo(con, columns, numCols, sPrinterName);
		  						//System.out.println("After y check");
		  					} else {
		  						//System.out.println("y == " + y + " and calling FileUploadBean.getDeviceInfo");
		  						dataInfoOld[y] = TheBean.getDeviceInfo(con, columns, numCols, sDeviceName);
		  						//System.out.println("After y= check");
		  					}
	  					} else {
	  						dataInfoOld[y][1] = "Device does not exist";
	  					}
	  				} catch (Exception e) {
	  					System.out.println("GPWS error in AnalyzeMassUpdate.jsp.11a ERROR: " + e);
	  				}
	  				System.out.println("After FileUploadBean.getDeviceInfo catch");
  				}
  			}
				...
				if (columns[count].equals("GEO")) {
					...
					try {
						...
						while (rsLoc.next()) {
	  					...
	  					System.out.println("GPWS error in AnalyzeMassUpdate.jsp.S1 ERROR: " + e);
	  				} finally {
	  					try {
	  						if (rsLoc != null)
	  							rsLoc.close();
		  					...
		  					System.out.println("GPWS Error in AnalyzeMassUpdate.jsp.S2 ERROR: " + e);
	  					}
	  				}
	  				if (iGeoID != 0) {
	  					sSQLServCols += "GEOID,";
				...
				if (columns[count].equals("ENBLHDRID")) {
					...
					try {
						...
						if (bSAP == false) {
	  					...
	  					System.out.println("GPWS error in AnalyzeMassUpdate.jsp.S3 ERROR: " + e);
	  				} finally {
	  					try {
	  						if (rsStatus != null)
	  							rsStatus.close();
		  					...
		  					System.out.println("GPWS Error in AnalyzeMassUpdate.jsp.S4 ERROR: " + e);
	  					}
	  				}
	  				if (bSAPCheck == true) {
	  					sSQLEnabCols += columns[count] + ",";
...
<%		sql = sb.toString();
			...
			if (Table.equals("DEVICE") && y > 0) {
				if (printerCheck == true) {
					...
					try {
						...
						System.out.println("ERROR in AnalyzeMassUpdate.jsp.Insert: " + e);
						appTool.logError("AnalyzeMassUpdate.jsp","GPWSAdmin",e);
	...
	<!-- </table>-->
<%	if (overMax == true) { %>
		<p><a class='ibm-error-link' href='#'></a><%= messages.getString("warning_only_first") %> <%= maxNumEntries - 1 %> <%= messages.getString("entries_executed_rest_ignored") %></p>
<%	}
	if (Action.equals("INSERT_UPDATE")) { 
		if (rowcount == 1) { %>
			<br /><%= rowcount %> <%= messages.getString("row_will_be_successfully_updated") %>
<%		} else { %>
			<br /><%= rowcount %> <%= messages.getString("rows_will_be_successfully_updated") %>
<%		}
	} else if (Action.equals("INSERT")) {
		if (rowcount == 1) { %>
			<br /><%= rowcount %> <%= messages.getString("row_will_be_successfully_inserted") %>
<%		} else { %>
			<br /><%= rowcount %> <%= messages.getString("rows_will_be_successfully_inserted") %>
<%		}
	} else { %>
		<br /><%= rowcount %> <%= messages.getString("rows_will_be_successfully_updated") %>
<%	} 
		if (rowcounterror == 1) { %>
			<br /><%= rowcounterror %> <%= messages.getString("row_will_be_unsuccessful") %>
<%		} else { %>
			<br /><%= rowcounterror %> <%= messages.getString("rows_will_be_unsuccessful") %>
<%		}
		if (lineignore == 1) { %>
			<br /><%= lineignore %> <%= messages.getString("row_will_be_ignored") %>
<%		} else { %>	
			<br /><%= lineignore %> <%= messages.getString("rows_will_be_ignored") %>
<%		} 

		//***********************************************
		// Here is where we loop through and essentially print out the csv file %>
		<form action="<%= prtgateway %>" name="massUpdateForm" id="massUpdateForm" class="ibm-column-form ibm-styled-form" method="post">
				<div id='topageid'></div>
				<input type="hidden" name="<%= BehaviorConstants.TOPAGE %>" value="531"/>
				<input type="hidden" name="file" value="<%= TheBean.getFilename() %>" />
				<input type="hidden" name="upload" value="false" />
				<input type="hidden" name="filetype" value="<%= TheBean.getFileType() %>" />
				<input type="hidden" name="action" value="<%= TheBean.getFieldValue("action") %>" />
				<input type="hidden" name="table" value="<%= TheBean.getFieldValue("table") %>" />
		<br />
		<table border="0" cellpadding="0" cellspacing="0" class="ibm-data-table ibm-alternate-two ibm-table-scroll" summary="Print out of the csv file">
			<caption><em></em></caption>
			<thead>
				<tr>
					<th scope="col"><%= messages.getString("success") %></th>
					<th scope="col"><%= messages.getString("line_#") %></th>
	<%			for (int xy = 1; xy < numCols; xy++) { %>
					<th><%= columns[xy] %></th>
	<%			} %>
				</tr>
			</thead>
			<tbody>
<%			willFail = false;
			for (int yz = 1; yz < x; yz++) {
				willFail = false;
				for (int mc = 0; mc < errormsgs.length; mc++) {
					if(errormsgs[mc] != null && errormsgs[mc].startsWith(dataInfo[yz][0] + ":")) {
						willFail = true;
					} 
			   	}
				
				if ( Action.equals("INSERT_UPDATE") && (tool.nullStringConverter(dataInfoOld[yz][1]).equals("Device does not exist") || tool.nullStringConverter(dataInfoOld[yz][1]).equals("Could not query db")) ) {
					willFail = true;
				}
				if (Action.equals("INSERT")) { %>
				    <tr><td><% if (willFail) { %><p><a class='ibm-error-link' href='#'></a></p> <% } else { %><p><a class='ibm-confirm-link'></a></p><% } %></td>
<%				} else { %>
				<tr><td><% if (willFail) { %><p><a class='ibm-error-link' href='#'></a></p> <% } else { %><p><a class='ibm-confirm-link'></a></p><% } %></td>
<%				}
				for (int xy = 0; xy < numCols; xy++) { %>
				<td><%= dataInfo[yz][xy] %></td>
<%				} %>
			</tr>
<%				if (Action.equals("INSERT_UPDATE")) { %>
				<tr><td></td>
<%					for (int xy = 0; xy < numCols; xy++) { 
						if (tool.nullStringConverter(dataInfoOld[yz][xy]).equals("Device does not exist") || tool.nullStringConverter(dataInfoOld[yz][xy]).equals("Could not query db")) { %>
				<td colspan="2"><%= tool.nullStringConverter(dataInfoOld[yz][xy]) %></td>
<%						} else { %>
				<td><%= tool.nullStringConverter(dataInfoOld[yz][xy]) %></td>
<%						}
					}
				}
			} //for %>
...
<%		//************************************************
		 if (rowcounterror > 0) { %>
				<p><b><%= messages.getString("errors") %>:</b></p>
			<% for (int mc = 0; mc < errormsgs.length; mc++) {
					if(errormsgs[mc] != null) { %>
						Line <%= errormsgs[mc] %><br />
			<%		}
			   } //for
		 }
		 if (rowcountwarning > 0 || bRCWarning == true) { %>
				<p><b><%= messages.getString("warnings") %>:</p>
...
%>
			<br />
				...
				<div class="ibm-buttons-row">
					<p align="right">
						<span>
							<% if (rowcounterror == 0 && !willFail) { %>
							<input value="<%= messages.getString("submit") %>" type="submit" name="ibm-submit" class="ibm-btn-arrow-pri" />
							<% } %>
...
<%
	...
	System.out.println("ERROR in AnalyzeMassUpdate.jsp: " + e);
	appTool.logError("AnalyzeMassUpdate.jsp","GPWSAdmin",e);
%>
	<br /><%= rowcount %> <%= messages.getString("row(s)_will_be_completed_successfully") %><br />
