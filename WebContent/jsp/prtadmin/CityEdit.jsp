<%
	TableQueryBhvr CityTableView  = (TableQueryBhvr) request.getAttribute("CityTableView");
	TableQueryBhvrResultSet CityTableView_RS = CityTableView.getResults();
	...
	String status = "";
	CityTableView_RS.next();
	    CityID	= CityTableView_RS.getInt("CITYID");
	    stateid = CityTableView_RS.getInt("STATEID");
		status = tool.nullStringConverter(CityTableView_RS.getString("CITY_STATUS"));
		city = tool.nullStringConverter(CityTableView_RS.getString("CITY"));

%>

	<%@ include file="metainfo.jsp" %>
	<meta name="Keywords" content="Global Print edit city"/>
	<meta name="Description" content="Global print website edit city information" />
	<title><%= messages.getString("title") %> | <%= messages.getString("city_edit") %></title>
	...
	<script type="text/javascript">
	 ...
	 function addSite(event) {
	 	...
	 	dojo.stopEvent(event);
	 		if (formValid) {
	 			if (!found) {
		 			logaction.value = "Site " + wName + " has been updated";
					formName.submit();
				} else {
	 ...
	 dojo.ready(function() {
     	createHiddenInput('topageid','<%= BehaviorConstants.TOPAGE %>', '196');
        ...
        createpTag();
        createTextInput('site','city','site','64',true,'<%= messages.getString("required_info") %>','required','<%= messages.getString("field_problems") %>',g_loc_city_regexp,'<%= city %>');
	...
	<div id="ibm-leadspace-head" class="ibm-alternate">
		<div id="ibm-leadspace-body">
			...
			<h1><%= messages.getString("city_edit") %></h1>
		</div>
	</div>
