	...
	<meta name="Keywords" content="Global Print add country"/>
	<meta name="Description" content="Global print website add country information" />
	<title><%= messages.getString("title") %> | <%= messages.getString("country_add") %></title>
	...
	<script type="text/javascript">
	 ...
	 function addCountry(event) {
	 	...
	 	dojo.stopEvent(event);
	 		if (formValid) {
	 			if (!found) {
		 			logaction.value = "Country " + wName + " has been added";
					formName.submit();
				} else {
	 ...
	 dojo.ready(function() {
     	createHiddenInput('topageid','<%= BehaviorConstants.TOPAGE %>', '407');
        createHiddenInput('logactionid','logaction','');
        createHiddenInput('logactionid','geo','<%=geo%>');
        createpTag();
        createTextInput('country','country','country','64',true,'<%= messages.getString("required_info") %>','required','<%= messages.getString("field_problems") %>',g_loc_country_regexp,'');
        createTextInput('countryabbr','countryabbr','countryabbr','2',true,'<%= messages.getString("required_info") %>','required','<%= messages.getString("field_problems") %>','\\w{0,2}','');
