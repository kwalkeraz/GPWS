<%
	com.ibm.aurora.bhvr.TableQueryBhvr InProgressSitesAdminView  = (com.ibm.aurora.bhvr.TableQueryBhvr) request.getAttribute("InProgressSitesAdminView");
	com.ibm.aurora.bhvr.TableQueryBhvrResultSet InProgressSitesAdminView_RS = InProgressSitesAdminView.getResults();
...
<meta name="keywords" content="Global Print, keyop, admin view open requests"/>
...
dojo.require("dijit.form.Button");
	...
	var info = new Array();
	dojo.ready(function() {
		createHiddenInput('nextpageid','next_page_id','3097');
 		createpTag();
...
<%--  		autoSelectValue('countryid','<%= sCountry %>'); --%>
	...
	function changeView(first) {
		self.location.href='<%= keyops %>?next_page_id=3097&cityid=<%= request.getParameter("cityid") %>&countryid=<%= request.getParameter("countryid") %>&ft=0&mpp=' + document.getElementById("view").value;
	}
	function addSites() {
	<%	int count1 = 0;
		while(InProgressSitesAdminView_RS.next()) { %>
			info[<%= count1 %>] = "<%= InProgressSitesAdminView_RS.getInt("COUNTRYID") %>" + "=" + "<%= InProgressSitesAdminView_RS.getString("CITY") %>" + "=" + "<%= InProgressSitesAdminView_RS.getInt("CITYID") %>";
	<%		count1++;
		} %>
	}
	...
	function addCountries() {
		...
		while (InProgressCountriesAdmin_RS.next()) { 
			cid = InProgressCountriesAdmin_RS.getInt("COUNTRYID");%>
			addOption('countryid','<%= InProgressCountriesAdmin_RS.getString("COUNTRY") %>','<%= cid %>');
		<%	c++;
			if (c == 1) { %>
	...
	try {
		...
		if (iCityID >= 0) {
			...
			if(iCityID != 0 && iCountryID != 0) {
				if (isKOSU == true) {
					rsReqCount = stmtReqCount.executeQuery("SELECT COUNT(*) AS COUNT FROM GPWS.KEYOP_REQUEST WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') AND CITYID = " + iCityID);
				} else {
					rsReqCount = stmtReqCount.executeQuery("SELECT COUNT(*) AS COUNT FROM GPWS.KEYOP_REQUEST_VIEW WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') AND CITYID = " + iCityID + " AND KO_COMPANYID = " + iVendorID);
				}
			} else if (iCityID == 0 && iCountryID != 0) {
				if (isKOSU == true) {
					rsReqCount = stmtReqCount.executeQuery("SELECT COUNT(*) AS COUNT FROM GPWS.KEYOP_REQUEST WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD')");
				} else {
					rsReqCount = stmtReqCount.executeQuery("SELECT COUNT(*) AS COUNT FROM GPWS.KEYOP_REQUEST_VIEW WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') AND KO_COMPANYID = " + iVendorID);
				}
			}
			rsReqCount.next();
			...
			if(iCityID != 0 && iCountryID !=0) {
				if (isKOSU == true) {
					rsNewReq = stmtNewReq.executeQuery("SELECT DISTINCT KEYOP_REQUESTID, REQUESTOR_EMAIL, REQUESTOR_NAME, REQUESTOR_TIELINE, REQUESTOR_EXT_PHONE, TIME_SUBMITTED, CITY, DEVICE_NAME, DEVICE_TYPE, BUILDING, FLOOR, ROOM, KO_COMPANYID, KO_COMPANY_NAME FROM GPWS.KEYOP_REQUEST_VIEW WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') AND CITYID = " + iCityID + " ORDER BY " + sOrderBy);
				} else {
					rsNewReq = stmtNewReq.executeQuery("SELECT DISTINCT KEYOP_REQUESTID, REQUESTOR_EMAIL, REQUESTOR_NAME, REQUESTOR_TIELINE, REQUESTOR_EXT_PHONE, TIME_SUBMITTED, CITY, DEVICE_NAME, DEVICE_TYPE, BUILDING, FLOOR, ROOM, KO_COMPANYID, KO_COMPANY_NAME FROM GPWS.KEYOP_REQUEST_VIEW WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') AND CITYID = " + iCityID + " AND KO_COMPANYID = " + iVendorID + " ORDER BY " + sOrderBy);
				}
			} else if (iCityID == 0 && iCountryID != 0) {
				if (isKOSU == true) {
					rsNewReq = stmtNewReq.executeQuery("SELECT DISTINCT KEYOP_REQUESTID, REQUESTOR_EMAIL, REQUESTOR_NAME, REQUESTOR_TIELINE, REQUESTOR_EXT_PHONE, TIME_SUBMITTED, CITY, DEVICE_NAME, DEVICE_TYPE, BUILDING, FLOOR, ROOM, KO_COMPANYID, KO_COMPANY_NAME FROM GPWS.KEYOP_REQUEST_VIEW WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') ORDER BY " + sOrderBy);
				} else {
					rsNewReq = stmtNewReq.executeQuery("SELECT DISTINCT KEYOP_REQUESTID, REQUESTOR_EMAIL, REQUESTOR_NAME, REQUESTOR_TIELINE, REQUESTOR_EXT_PHONE, TIME_SUBMITTED, CITY, DEVICE_NAME, DEVICE_TYPE, BUILDING, FLOOR, ROOM, KO_COMPANYID, KO_COMPANY_NAME FROM GPWS.KEYOP_REQUEST_VIEW WHERE (REQ_STATUS = 'in progress' OR REQ_STATUS = 'HOLD') AND KO_COMPANYID = " + iVendorID + " ORDER BY " + sOrderBy);
				}
			}
		}
%>
...
%>
				<p class="ibm-table-navigation"><span class="ibm-primary-navigation">
					...
					<% } %>
					<span class="ibm-table-navigation-links"> <% if (iFirst > 0) { %>| <a class="ibm-back-em-link" href="<%= keyops %>?next_page_id=3097&cityid=<%= request.getParameter("cityid") %>&countryid=<%= request.getParameter("countryid") %>&page=<%= iPage - 1%>"><%= messages.getString("previous") %></a> <% } if (iReqMax < iNumReq) { %>| <a class="ibm-forward-em-link" href="<%= keyops %>?next_page_id=3097&cityid=<%= request.getParameter("cityid") %>&countryid=<%= request.getParameter("countryid") %>&page=<%= iPage + 1%>"><%= messages.getString("next") %></a><% } %></span></p>
				...
				<table cellspacing="0" cellpadding="0" border="0" class="ibm-data-table ibm-sortable-table" summary="<%= messages.getString("list_of_inprog_keyop_requests") %>">
					<caption><em><%= messages.getString("keyop_requests_at_site") %>:&nbsp;&nbsp;<%= tool.getSiteName(iCityID) %></em></caption>
						<thead>
							<tr>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByTicketNum %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("ticket") %></a></th>
								<% if (isKOSU == true) { %>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByKOCompany %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("vendor_name") %></a></th>
								<% } %>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByReqName %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("opened_by") %></th>
								<th scope="col"><%= messages.getString("tieline") %></th>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByTimeSubmit %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("time_opened") %></a></th>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByDevice %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("device") %></a></th>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByDeviceType %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("type") %></a></th>
								<th scope="col"><a href="<%= keyops %>?next_page_id=3097&orderby=<%= sOrderByCity %>&countryid=<%= iCountryID %>&cityid=<%= iCityID %>&page=<%= iPage %>"><%= messages.getString("site") %></a></th>
					...
					<%	}
							...
							System.out.println("Keyop error in AdminViewOpenReq.jsp ERROR: " + e); %>
