...
<title><%= messages.getString("global_print") %> | <%= messages.getString("keyop_admin_page") %> - <%= messages.getString("add") %>&nbsp;<%= messages.getString("keyop") %></title>
		...
		load : function(response, args) {
				...
				if (sLook == "lookUpAdmin"){ // look up in database
					...
					if (Auth != "" && Auth != null && Auth == "Keyop") {
						dojo.byId("errorMsg").innerHTML = '<a class="ibm-error-link" href="#"></a><%= messages.getString("keyop_already_exist") %>';
						dijit.byId('firstname').set('value','');
						dijit.byId('lastname').set('value','');
						dijit.byId('email').set('value','');
						dijit.byId('loginid').set('value','');
					} else if (Auth != "" && Auth != null && (Auth == "Keyop Admin" || Auth == "Keyop Superuser")) {
...
dojo.ready(function() {
	createHiddenInput('next_page_id', 'next_page_id','3025');
	createHiddenInput('submitvalue', 'submitvalue','AddKeyop');
	createHiddenInput('userid', 'userid','');
	createpTag();
	createTextInput('lookupEmail','lookupEmail','lookupEmail','64',false,'','','<%= messages.getString("field_problems") %>',g_email_regexp,'');
	...
	createTextInput('pager','pager','pager','64',false,'','','<%= messages.getString("field_problems") %>',g_email_regexp,'');
    ...
    <% if (isSuperUser == true) { %>
		createSelect('vendor', 'vendor', '<%= messages.getString("please_select_option") %>', '0', 'vendor');
		loadVendorValues();
    ...
    <%if (!error.equals("")){ %>
		dojo.byId("response").innerHTML = "<p class='ibm-error'><a class='ibm-error-link'></a>"+"<%= error %>"+"<br /></p>";
		dojo.byId("response-hr").style.display = "";
	<% } %>
});
...
function addKeyop() {
	...
	if (validForm) {
		<% if (isSuperUser == true) { %>
			if (!showSelectMsg("0", dijit.byId('vendor').attr("value"), '<%= messages.getString("please_select_vendor") %>', 'vendor')) {
				return false;
			} else {
...
<div id="ibm-top" class="ibm-landing-page">
	...
	<div id="ibm-leadspace-head" class="ibm-alternate">
		<div id="ibm-leadspace-body">
			...
			<h1><%= messages.getString("add") %>&nbsp;<%= messages.getString("keyop") %></h1>
		</div> <!--  Leadspace-body -->
	</div> <!--  Leadspace-head -->
	<!-- LEADSPACE_END -->
			...
			<div id="ibm-content-body">
				<div id="ibm-content-main">
					...
					<div id="theForm">
						...
						<div id="userid"></div>
						<div class="pClass">
							<label for="lookupEmail"><%= messages.getString("email") %>:</label>
						...
						<% if (isSuperUser == true) { %>
						<div class="pClass">
							<label for="vendor"><%= messages.getString("vendor") %>:<span class="ibm-required">*</span></label>
							<span><div id="vendor"></div></span>
						</div>
						<% } %>
						<div class="pClass">
							<label for="pager"><%= messages.getString("pager_number") %>:&nbsp;<span class="ibm-item-note"><%= messages.getString("example_pagerno") %></span></label>
