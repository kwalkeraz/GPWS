...
function analyzeBrowser() {
	...
	} else if (checkOSBrowser("trident") >= 0) {
		browser = "Internet Explorer";
		bversion = getBrowserVersion('trident');
...
function getBrowserVersion(browserValue) {
	...
	if (browserValue == 'msie') {
		...
		if (userAgentValue.indexOf('trident/4.0') > 0) {
			browserVer = "8.0";
		} else if (userAgentValue.indexOf('trident/5.0') > 0) {
			browserVer = "9.0";
		} else if (userAgentValue.indexOf('trident/6.0') > 0) {
			browserVer = "10.0";
		} else if (userAgentValue.indexOf('trident/7.0') > 0) {
			browserVer = "11.0";
		} else if (userAgentValue.indexOf('trident/8.0') > 0) {
			browserVer = "12.0";
		}
	} else if (browserValue == 'trident') {
		if (userAgentValue.indexOf('trident/4.0') > 0) {
			browserVer = "8.0";
		} else if (userAgentValue.indexOf('trident/5.0') > 0) {
			browserVer = "9.0";
...
function getWinVer() {
	...
	} else if (spotVer == '6.2' || spotVer == '6.3') {
		winver = 'Windows 8';
...
function getLinVer() {
	...
	} else if (userAgentValue.indexOf('ubuntu') >= 0) {
		linVer = "Ubuntu";
	} else {
		linVer = "Linux";
	}
...
function getMacVer() {
	if (userAgentValue.indexOf('os x') >= 0) {
		...
		macVer = "Mac OS X " + userAgentValue.substring(spot + 5,userAgentValue.indexOf(';',spot));
	} else {
		macVer = 'Mac';
	}
	return macVer;
