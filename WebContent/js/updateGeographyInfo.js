...
function updateGeo(selectedValue,deviceavailable, completeStatus) {
	...
	if (dijit.byId("city")) resetMenu('city');
 	...
 	params = (completeStatus != "" && completeStatus != undefined) ? params + "&status="+completeStatus: params;
 ...
 function updateCountry(selectedValue,deviceavailable,completeStatus) {
	if (dijit.byId("floor")) resetMenu('floor');
	if (dijit.byId("building")) resetMenu('building');
	if (dijit.byId("city"))resetMenu('city');
 	if (dijit.byId("state")) resetMenu('state');
 ...
 function updateState(selectedValue,deviceavailable,completeStatus) {
	if (dijit.byId("floor")) resetMenu('floor');
	if (dijit.byId("building")) resetMenu('building');
	if (dijit.byId("city")) resetMenu('city');
 	resetMenu('state');
 ...
 function updateCity(selectedValue,deviceavailable,completeStatus) {
	resetMenu('floor');
 	resetMenu('building');
 	resetMenu('city');
	...
	url = "/tools/print/servlet/printeruser.wss?to_page_id=10000&" + params;
 	...
 	if (state != "None" && state != "0" && state != "*") {
 		getXMLData(url,tagName,dataTag,dID,selectedValue, true);
 	}
 } //end updateCity
 
 function updateBuilding(selectedValue,deviceavailable,completeStatus) {
	resetMenu('floor');
 	resetMenu('building');
 ...
 function updateFloor(selectedValue,deviceavailable,completeStatus) {
	resetMenu('floor');
